...
max_obstacles = 40 # Number of obstacles that will try to spawn 
debug = True # Change to show extra print statements 
distance_from_front = 6 # Distance between front vehicle  
...
class VehicleMonitor:
    ...
    def _move_to_nearest_road(self):
        ...
        if self.front:
            waypoints_ahead = waypoint.previous(10)
            wp = waypoints_ahead[0]
            self.other_vehicle.set_transform(wp.transform)
        else:           
            waypoints_ahead = waypoint.next(10)
            wp = waypoints_ahead[0]
            self.other_vehicle.set_transform(wp.transform)
...
def back_control_update(world, frontvehicle, backvehicle, back_controller):
    ...
    if len(waypoints_behind) > 0:
        ...
        #print(back_location.distance(frontvehicle.get_location()))
        print(current_distance)
        #print(back_location)
        #print(target_waypoint.transform.location)
        desired_speed = max(0, current_distance) * 3.6 * frontspeed/10 # Convert to km/h
...
################################################################################################################
        ...
        frontvehicle = world.try_spawn_actor(altfrontvehicle_bp[0], random_spawn_point)
        ...
        if frontvehicle:
            ...
            if backvehicle:
                ...
                frontvehicle = world.try_spawn_actor(altfrontvehicle_bp[0], new_transform)
