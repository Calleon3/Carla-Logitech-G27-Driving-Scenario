...
width = 800 # 1920 #pygame screen width
height = 600 # 1080 #pygame screen height
...
def back_control_update(world, frontvehicle, backvehicle, back_controller, controller):
    ...
    waypoints_behind = front_waypoint.previous(5)    
    ...
    if len(waypoints_behind) > 0:
        ...
        controller.play_spring_force(0,int(steering*100),100,100)  
...
################################################################################################################
        ...
        camera_transform = carla.Transform(carla.Location(x=0.4, z=1.25), camera_rotation)
...
################################################################################################################
        ...
        while True:
            ...
            back_control_update(world, frontvehicle, backvehicle, back_controller, logi_controller)
...
writeText("Press Space to intialise")
while True:    
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            pygame.quit()
