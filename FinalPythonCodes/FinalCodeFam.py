...
max_obstacles = 0 # Number of obstacles that will try to spawn (limit is about 20-25)
distance_from_front = 10 # Distance the PID will attempt to keep between cars (over 10 can cause weird behaviour with junctions) 
...
TIMEOUT = 300*1000  # Time in seconds * 1000 = milliseconds 
...
def back_control_update(world, frontvehicle, backvehicle, back_controller, logi_controller, changing_lane):
    ...
    if len(waypoints_behind) > 0:
        ...
        # Calculate desired speed based on distance to target waypoint 
        desired_speed = max(0, dist) * 3.6 * frontspeed/15 # Convert to km/h and normalises for front car speed limit
        #Add a max desired speed 
        if desired_speed > 35:
            desired_speed = 35
        ...
        if wheelcontrol:
            ...
            #control.steer = 1.18*(np.sin(get_steering_input()))
            #control.steer = get_steering_input() * (1/14)

            #print(get_steering_input())
            #print(get_steering_input() * (1/14))
...
def spawnObstacles(world, actor_list, spawn_points, obstacle_bp):
    ...
    while num_obstacles < max_obstacles:
        ...
        if min_distance >= 40 and min_dist_corner >= 20:
